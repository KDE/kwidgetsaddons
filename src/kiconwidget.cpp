...
QSize KIconWidgetPrivate::effectiveIconSize() const
{
    if (!iconSize.isEmpty()) {
        ...
        const int largeIconSize = q->style()->pixelMetric(QStyle::PM_LargeIconSize);
...
{
    setSizePolicy(QSizePolicy(QSizePolicy::Preferred, QSizePolicy::Preferred, QSizePolicy::Label));
    d->icon = icon;
    d->iconSize = iconSize;
}
